...
def _ollama_suggest(trip, preferences: Dict[str, Any], max_items: int) -> Dict[str, Any]:
    ...
    try:
        # Ensure model exists; try to pull if missing
        show = requests.post(
        ...
        if show.status_code == 404:
            ...
            if pull.status_code not in (200, 201):
                return {"activities": [], "provider": "ollama", "error": f"Ollama pull failed: {pull.status_code}"}
        ...
        return {"activities": [], "provider": "ollama", "error": f"Ollama connection error: {exc}"}
...
def suggest_activities(trip, preferences: Dict[str, Any], max_items: int = 8) -> Dict[str, Any]:
    ...
    """
    # Preferred provider order: explicit LLM_PROVIDER else Ollama -> OpenAI -> fallback
    provider = (os.getenv("LLM_PROVIDER") or "").lower().strip()

    # Try local Ollama first unless provider is explicitly 'openai'
    if provider != "openai":
    ...
    if provider == "openai" and not api_key:
        # If explicitly openai but no key, attempt ollama as soft fallback
        oll = _ollama_suggest(trip, preferences, max_items)
        if oll and oll.get("activities"):
            return oll
        return _fallback_suggestions(trip, preferences, max_items) | {"error": "OPENAI_API_KEY not set"}
    if not api_key and provider != "openai":
        # No key and not forced openai -> we already tried Ollama above
    ...
    try:
        ...
        if resp.status_code != 200:
            ...
            # On OpenAI failure (e.g., 429 insufficient_quota), try Ollama before final fallback
            oll = _ollama_suggest(trip, preferences, max_items)
            if oll and oll.get("activities"):
                return oll | {"error": f"OpenAI error {resp.status_code}: {detail}"}
        ...
        # If OpenAI call threw an exception, try Ollama once more
        oll = _ollama_suggest(trip, preferences, max_items)
        if oll and oll.get("activities"):
            return oll | {"error": str(exc)}
