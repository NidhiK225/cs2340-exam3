...
def _build_prompt(latitude:float, longitude:float, preferences: Dict[str, Any], max_items: int) -> str:
    ...
    lines = [
        ...
        f"- Target Latitude: {latitude}",
        f"- Target Longitude: {longitude}",
        "",
        "Planner preferences:",
...
def suggest_activities(latitude:float, longitude:float, preferences: Dict[str, Any], max_items: int = 8) -> Dict[str, Any]:
    ...
    """
    prompt = _build_prompt(latitude, longitude, preferences, max_items)
    ...
    def try_gemini():
        return gemini_provider.suggest(latitude, longitude, prompt, max_items=max_items)

    def try_openai():
        return openai_provider.suggest(latitude, longitude, prompt, max_items=max_items)
    ...
    local = local_provider.suggest(latitude, longitude, preferences, max_items=max_items)
